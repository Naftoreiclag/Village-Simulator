...
public class SMDNode extends SkinNode
{
	...
	private static final Logger logger = Logger.getLogger(SMDNode.class.getName());
	...
	public SMDNode(String resourceQCFile, DisplaySystem display)
	{
		...
		this.setLocalRotation(this.buildMatrix(new Vector3f(((float) (Math.PI + Math.PI / 2)), 0, 0)));
	}
	 ...
	 * 
	 * @param resourceQCFile
	 *            : String with the path of .qc file that contains models data.
	 */
	private void getSmdNode(String resourceQCFile)
	{
		...
		try
		{
			...
			for (int i = 0; i < meshes.size(); i++)
			{
				...
				{// add the meshes to the node.
					logger.log(Level.INFO, "Skinning, for mesh : " + model.getName() + i + j);
					this.addSkin(buildTrimesh(model.getName() + i + j, model.getMeshes().get(j)));
				}
	 ...
	 *            : Mesh name.
	 * @param mesh
	 *            : SmdMesh that contains the vertex, normals and texture
	 *            coordinates.
	 ...
	 * 
	 * @param animations
	 *            : SmdAnimations to build the animation controller.
	 */
		...
		try
		{
			...
			for (int i = 0; i < animations.size(); i++)
			{
				if (animations.get(i).isBlendAnimation())
				{
					...
					{
						controller.addAnimation(buildBoneAnimation(animations.get(i).getSmdAnimation(j), this.getSkeleton()));
					}
				}
				else
				{
					controller.addAnimation(buildBoneAnimation(animations.get(i), this.getSkeleton()));
				}
			}
			logger.log(Level.INFO, "Animation controller created OK, animation " + "number : " + controller.getAnimations().size() + ".");
		}
		catch (Exception Ex)
		{
	 ...
	 *      traslation/rotation for each time. Creates a bone Transformation Map
	 *      that holds the bone and the matrix position for each time for that
	 *      bone.
	 * @param animation
	 *            : SmdAnimation parsed from Smd animation file, contains the
	 *            bone strucure and bone traslation/rotation for each time.
	 * @param skeleton
	 *            builded bone skeleton.
	...
	private BoneAnimation buildBoneAnimation(SmdAnimation animation, Bone skeleton)
	{
		...
		try
		{
			logger.log(Level.INFO, "Trying to create animation : " + animation.getName());
			...
			logger.log(Level.INFO, "Added times.Animation fps : " + animation.getFps());
			...
			logger.log(Level.INFO, "Total animation frames : " + (animation.getSkeletonPositions().length - 1));
			banimation.setEndFrame(animation.getSkeletonPositions().length - 1);
		}
		catch (Exception Ex)
		{
			logger.log(Level.SEVERE, "Error, creating Animation " + animation.getName() + " : ", Ex);
		}
		return banimation;
	}
	...
	/**
	 * Method that adds a transform to the bone transformation list. Creates a a
	 * Bind Matrix from SmdBonePosition.
	 * 
	 * @param sboneTrans
	 *            : Object that stores the bone transformation.
	 * @param bone
	 *            : bone that affect transformation.
	 * @param time
	 *            : frame of the bone transform.
	 */
	 ...
	 * 
	 * @param boneToTransform
	 *            : bone to add transform.
	 * @param relPositions
	 *            : ArrayList with Matrix4f positions of the bone.
	...
	/**
	 * Method that builds float[] with animation time position. Calculates the
	 * time between frames.
	 * 
	 * @param fps
	 *            : float with frames per second of the animation.
	 * @param positions
	 *            : int animation positions number.
	...
	/**
	 * Method that applies a texture to a mesh. builds a texture path with qc
	 * file and texture name.
	 * 
	 * @param mesh
	 *            : Trimesh to apply texture.
	 * @param textureName
	 *            : String with the texture name to apply.
	 */
	private void setTexture(TriMesh mesh, String textureName)
	{
		...
		try
		{
			logger.log(Level.INFO, "Trying to apply texture : " + textureName + " to mesh : " + mesh.getName());
			textFile = new File(textureURL.concat(File.separator).concat(textureName));
			TextureState ts = display.getRenderer().createTextureState();
			Texture t1 = TextureManager.loadTexture(textFile.toURI().toURL(), Texture.MinificationFilter.Trilinear, Texture.MagnificationFilter.Bilinear);
			ts.setTexture(t1, 0);
	 ...
	 * 
	 * @param skeleton
	 *            : SmdSkeleton that contain the skeleton and the bones.
	 */
	 ...
	 * 
	 * @param sbones
	 *            : SmdBones with influence data (geometric Intex, vetex).
	 */
			...
			for (int i = 0; i < sbones.length; i++)
			{
				...
				if (bv != null)
				{
					for (int j = 0; j < bv.size(); j++)
					{
						...
						{
							logger.log(Level.INFO, "FAILS : " + bv.get(j).getGeomIndex() + " " + sbones[i].getBoneName() + " " + bv.get(j).getVertIndex());
	 ...
	 * 
	 * @param tras
	 *            : Vector3f with traslation.
	 * @param rot
	 *            : Vector3f with rotation.
	...
	private Quaternion buildQuat(Vector3f rot)
	{
		...
		{
			logger.log(Level.SEVERE, "Error, building Quaternion from rotation" + " Matrix.", Ex);
		}
		return q;
	}
	...
	private Matrix3f buildMatrix(Vector3f rot)
	{
		...
		{
			logger.log(Level.SEVERE, "Error, building rotation Matrix3f from " + "rotation vector.", Ex);
		}
