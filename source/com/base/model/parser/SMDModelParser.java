...
public class SMDModelParser extends SMDParser
{
	...
	private String type;
	private static final Logger logger = Logger.getLogger(SMDModelParser.class.getName());
	...
	/**
	 * Method that parses .smd file, recovers the triangles, meshes and
	 * skeleton.
	 * 
	...
	public SmdModel parse()
	{
		...
		{
			logger.log(Level.SEVERE, "Error parsing .smd file " + filePath + " : ", Ex);
		}
		return model;
	}
	...
	private String getVersion(String line)
	{
		...
		{
			logger.log(Level.SEVERE, "Error recovering version from line" + " tokens : " + line, Ex);
		}
		return version;
	}
	...
	private void getBones(String line)
	{
		...
		{
			logger.log(Level.SEVERE, "Error recovering bones from line" + " tokens : " + line, Ex);
		}
	}

	/**
	 * Method that recovers bone position (rotation and traslation) from initial
	 * pose, stores bone positions in a list.
	 * 
	 * @param line
	 *            String with the line to parse.
	 */
	private void getBonePositions(String line)
	{
		...
		{
			logger.log(Level.SEVERE, "Error recovering bone positions from line" + " tokens : " + line, Ex);
		}
	}
	 ...
	 * 
	 * @param line
	 *            : String text line from smd file.
	 */
	private void getMeshes(String line)
	{
		...
		if (!line.startsWith(TRIANGLES_TAG))
		{
			...
			if (lparts.length == 1)
			{// checks texture name
				// creates new submesh with a new geomIndex, and initialize the
				// vert index.
	 ...
	 * 
	 * @param line
	 *            : String with text line to analize.
	 * @param geomIndex
	 *            : mesh index that triangles composes.
	 */
	private void getTriangles(String[] splitLine, int geomIndex)
	{
		...
		{
			logger.log(Level.SEVERE, "Error recovering triangle from geomIndex : " + geomIndex, Ex);
		}
	}

	/**
	 * Method that recovers a bone from bones map with vertex bone index and
	 * assigns the vertex.
	 * 
	 * @param vert
	 *            SmdVertex to add.
	...
	private void buildBoneInfluence(SmdVertex vert)
	{
		...
		{
			logger.log(Level.SEVERE, "Error adding bone influence from " + "geomIndex : " + geomIndex, Ex);
		}
	}
	...
	private SmdSkeleton buildSkeleton()
	{
		...
		{
			logger.log(Level.SEVERE, "Error building skeleton from " + "geomIndex : " + geomIndex, Ex);
		}
		return smdskeleton;
	}

	/**
	 * Method that builds submeshes, stores a submesh in a list of SmdMesh when
	 * triangle texture changes.
	 * 
	...
	private ArrayList<SmdMesh> buildMeshes()
	{
		...
		{
			logger.log(Level.SEVERE, "Error building meshes from geomIndex : " + geomIndex, Ex);
		}
		return this.meshes;
	}
}
