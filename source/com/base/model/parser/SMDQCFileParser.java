...
public class SMDQCFileParser extends SMDParser
{
	...
	private static final Logger logger = Logger.getLogger(SMDQCFileParser.class.getName());

	/**
	 * Instantiates new .qc file parser.
	 * 
	 * @param path
	 *            String with .qc file path.
	 */
	...
	/**
	 * Method that parses .qc file, recovers model meshes, animations and hit
	 * boxes.
	 * 
	 ...
	 * model and the type.SMDModelParser class needs geometric index to assing
	 * mesh to vertex.
	 * 
	 * @param path
	 *            : String with the path of the .smd file that contains the
	 *            model.
	 * @param type
	 ...
	 * and buils SmdAnimation object parsing smd animation file @see
	 * SMDAnimationParser
	 * 
	 * @param line
	 *            : String with sequence line from qc file.
	...
	private SmdAnimation getSmdAnimation(String line)
	{
		...
		try
		{
			...
			{
				if (lparts.length == 8)
				{// contains simple animation
					aFile = new File(folderPath.concat(File.separator).concat(lparts[2].concat(".smd")));
				}
				else if (lparts.length == 3)
				{// Main blend animation
				...
				if (aFile != null)
				{
					...
					{
						logger.log(Level.SEVERE, "Error, animation file : " + lparts[1] + " not found.");
					}
				}
			}
			else
			{// blend animation
				if (lparts.length == 1)
				{
					...
					{
						aFile = new File(folderPath.concat(File.separator).concat(lparts[0].concat(".smd")));
					}
				}
				else
				{
		...
		{
			logger.log(Level.SEVERE, "Error recovering animation from : " + line, Ex);
		}
		finally
		{
			anparser = null;
	 ...
	 * 
	 * @param line
	 *            : String line that contains SmdHbox
	...
	private SmdHbox buildSmdHitBox(String line)
	{
		...
		try
		{
			lparts = line.split(SPLITCHAR);
			lparts = this.clean(lparts, "\"");
			hbox = new SmdHbox(Integer.parseInt(lparts[1]), lparts[2], new Vector3f(Float.parseFloat(lparts[3]), Float.parseFloat(lparts[4]), Float.parseFloat(lparts[5])), new Vector3f(Float.parseFloat(lparts[6]), Float.parseFloat(lparts[7]), Float.parseFloat(lparts[8])));
		}
		catch (Exception Ex)
		{
			logger.log(Level.SEVERE, "Error recovering hitBox from : " + line, Ex);
		}
		return hbox;
	}
	 ...
	 * 
	 * @param bfilter
	 *            : String with the mesh name to filter.
	 */
