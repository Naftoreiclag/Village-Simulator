...
public class StringUtils
{
	...
	// ----------------------------------------------------------------------
	public static void printErrMsg(String methodName, String errorMsg,
			int mCount, char message[])
	{
			...
			String errMsg)
	{
		...
		if (errMsg != null)
		{
			...
			{
				printErrMsg("RString.skipWhiteSpace", errMsg, mCount,
						messageChars);
				return -1;
			}
		}
			...
			int startIndex)
	{
		...
		while (count < listLength)
		{
			...
			returnArray[returnArrayCount++] = Float.parseFloat(new String(
					listChars, itemStart, itemLength));
	...
	public static int[] parseIntList(String list, int startIndex)
	{
		...
		while (count < listLength)
		{
			...
			itemLength = itemEnd - itemStart;
			returnList.add(Integer.parseInt(new String(listChars, itemStart,
					itemLength)));
	...
	public static int[] parseListVerticeNTuples(String list,
			int expectedValuesPerTuple)
	{
		if (list == null)
		{
			return null;
		...
		for (int loopi = 0; loopi < vertexStrings.length; loopi++)
		{
			...
			parseVerticeNTuple(vertexStrings[loopi], returnList, emptyMarker,
					expectedValuesPerTuple);
		}
	...
	private static void parseVerticeNTuple(String list,
			ArrayList<Integer> returnList, Integer emptyMarker,
