...
public class Parse
{
	...
	private void parseObjFile(String objFilename) throws FileNotFoundException,
			IOException
	{
		int lineCount = 0;
		...
		while (true)
		{
			...
			{
				System.err.println("Parse.parseObjFile: line " + lineCount
						+ " unknown line |" + line + "|");
			}
			lineCount++;
		}
	...
	{
		float[] values = StringUtils.parseFloatList(3, line,
				OBJ_VERTEX.length());
	...
	{
		float[] values = StringUtils.parseFloatList(2, line,
				OBJ_VERTEX_TEXTURE.length());
	...
	{
		String[] groupnames = StringUtils.parseWhitespaceList(line.substring(
				OBJ_GROUP_NAME.length()).trim());
	...
	private void processMaterialLib(String line) throws FileNotFoundException,
			IOException
	{
		String[] matlibnames = StringUtils.parseWhitespaceList(line.substring(
				OBJ_MTLLIB.length()).trim());
		...
		if (null != matlibnames)
		{
			for (int loopi = 0; loopi < matlibnames.length; loopi++)
			{
				...
				{
					System.err
							.println("Parse.processMaterialLib: ERROR: Can't find material file name='"
									+ matlibnames[loopi] + "', e=" + e);
				}
			}
		}
	}
	...
	{
		builder.setCurrentUseMaterial(line.substring(OBJ_USEMTL.length())
				.trim());
	}

	private void processMapLib(String line)
	{
		String[] maplibnames = StringUtils.parseWhitespaceList(line.substring(
				OBJ_MAPLIB.length()).trim());
	...
	// ----------------------------------------------------------------------
	private void parseMtlFile(String mtlFilename) throws FileNotFoundException,
			IOException
	{
		int lineCount = 0;
		FileReader fileReader = null;
		...
		while (true)
		{
			...
			{
				System.err.println("Parse.parseMtlFile: line " + lineCount
						+ " unknown line |" + line + "|");
		...
		System.err
				.println("Parse.parseMtlFile: Loaded " + lineCount + " lines");
	}
	...
	private void processReflectivityTransmissivity(String fieldName, String line)
	{
		...
		String[] tokens = StringUtils.parseWhitespaceList(line
				.substring(fieldName.length()));
		if (null == tokens)
		{
			System.err
					.println("Parse.processReflectivityTransmissivity: ERROR! Got Ka line with no tokens, line = |"
							+ line + "|");
			return;
		}
		if (tokens.length <= 0)
		...
		if (tokens[0].equals("spectral"))
		{
			...
			System.err
					.println("Parse.processReflectivityTransmissivity: WARNING: Sorry Charlie, this parse doesn't handle \'spectral\' parsing.  (Mostly because I can't find any info on the spectra.rfl file.)");
			return;
			// if(tokens.length < 2) {
		...
		} else if (tokens[0].equals("xyz"))
		{
			...
			{
				System.err
						.println("Parse.processReflectivityTransmissivity: ERROR! Got xyz line with not enough x/y/z tokens, need at least one value for x, found "
								+ (tokens.length - 1)
								+ " line = |"
								+ line
								+ "|");
	...
	private void processIllum(String line)
	{
		...
		{
			System.err
					.println("Parse.processIllum: ERROR! Got illum model value out of range (0 to 10 inclusive is allowed), value="
