...
public class Build implements BuilderInterface
{
	...
	{
		System.err.println("Build.addPoints: @TODO: Got " + values.length
				+ " points in builder, ignoring");
	}

	public void addLine(int[] values)
	{
		System.err.println("Build.addLine: @TODO: Got a line of "
				+ values.length + " segments in builder, ignoring");
	}
	...
	public void addFace(int[] vertexIndices)
	{
		...
		while (loopi < vertexIndices.length)
		{
			...
			// System.err.println("Adding vertex g=" + vertexIndices[loopi] +
			// " t=" + vertexIndices[loopi + 1] + " n=" + vertexIndices[loopi +
			// 2]);
			int vertexIndex;
			...
			// rather than "the 5th vertice in the file" we can say
			...
			{
				System.err
						.println("Build.addFace: ERROR Index for geometric vertex="
								+ vertexIndex
			...
			if (vertexIndex != EMPTY_VERTEX_VALUE)
			{
				...
				{
					System.err
							.println("Build.addFace: ERROR Index for texture vertex="
									+ vertexIndex
			...
			if (vertexIndex != EMPTY_VERTEX_VALUE)
			{
				...
				{
					System.err
							.println("Build.addFace: ERROR Index for vertex normal="
									+ vertexIndex
			...
			{
				System.err
						.println("Build.addFace: ERROR Can't add vertex to face with missing vertex!  Throwing away face.");
				faceErrorCount++;
				return;
			}
	...
	public void addMapLib(String[] names)
	{
		...
		{
			System.err
					.println("Build.addMapLib: @TODO: ERROR! Got a maplib line with null names array - blank group line? (i.e. \"g\\n\" ?)");
			return;
		}
		if (names.length == 1)
		{
			System.err
					.println("Build.addMapLib: @TODO: Got a maplib line with one name=|"
							+ names[0] + "|");
			return;
		}
		...
		for (int loopi = 0; loopi < names.length; loopi++)
		{
			System.err.println("        names[" + loopi + "] = |"
					+ names[loopi] + "|");
		}
	}
	...
	// }
	// if (names.length == 1) {
	// System.err.println("Build.addMaterialLib: @TODO: Got a mtllib line with one name=|"
	// + names[0] + "|");
	// return;
	// }
	...
	{
		System.err.println("Build.doneParsing: Loaded filename '" + filename
				+ "' with " + verticesG.size() + " verticesG, "
				+ verticesT.size() + " verticesT, " + verticesN.size()
				+ " verticesN and " + faces.size() + " faces, of which "
				+ faceTriCount + " triangles, " + faceQuadCount
				+ " quads, and " + facePolyCount
